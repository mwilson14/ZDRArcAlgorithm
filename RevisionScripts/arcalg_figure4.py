...
from zdr_arc_section_fig4 import zdrarc_fig4
#from hail_section import hail_objects
from zhh_section import zhh_objects
from kdpfoot_section_fig12 import kdp_objects_fig12
#from zdr_col_section import zdrcol

def multi_case_algorithm_ML1_figure4(storm_relative_dir, zdrlev, kdplev, REFlev, REFlev1, big_storm, zero_z_trigger, storm_to_track, year, month, day, hour, start_min, duration, calibration, station, Bunkers_m, track_dis=10, GR_mins=1.0):
    ...
    g = Geod(ellps='sphere')
    #Open the placefile
    f = open("ARCALG_figure4"+station+str(dt.year)+str(dt.month)+str(dt.day)+str(dt.hour)+str(dt.minute)+"_Placefile.txt", "w+")
    ...
    #Local file option:
        ...
        for i in range(radar1.nsweeps):
            ...
            if ((np.mean(radar4.elevation['data']) < .65) and (np.max(np.asarray(radar4.fields['differential_reflectivity']['data'])) != np.min(np.asarray(radar4.fields['differential_reflectivity']['data'])))):
                ...
                refl_figure12 = radar.fields['reflectivity']['data']
                zdr_figure12a = radar.fields['differential_reflectivity']['data']
                
                zdr_figure12 = np.copy(zdr_figure12a)
                zdr_figure12 = ma.masked_where(refl_figure12 < 20, zdr_figure12)
                
                refl_figure12a = np.copy(refl_figure12)
                refl_figure12a = ma.masked_where(refl_figure12a < 20, refl_figure12a)
...
#                 states = ShapelyFeature(Reader(fname2).geometries(),ccrs.PlateCarree(), facecolor = 'none', edgecolor = 'black')
                ...
                refp = ax.pcolormesh(ungrid_lons, ungrid_lats, refl_figure12a, cmap=pyart.graph.cm_colorblind.HomeyerRainbow, vmin = 10, vmax = 73)
                ...
                if np.max(ZDRmasked) > zdrlev:
                    zdrc = ax.contour(rlons[0,:,:],rlats[0,:,:],ZDRmasked,zdrlev,linewidths = 4, colors='purple', alpha = 1.0)
                else:
                    zdrc=[]
...
#                     [hail_areas,hail_centroid_lon,hail_centroid_lat,hail_storm_lon,hail_storm_lat,ax,f] = hail_objects(hailc,REF_Hail2,ax,f,time_start,month,d_beg,h_beg,min_beg,sec_beg,d_end,h_end,min_end,sec_end,rlons,rlats,max_lons_c,max_lats_c,proj)
                    ...
                    [kdp_areas,kdp_centroid_lon,kdp_centroid_lat,kdp_storm_lon,kdp_storm_lat,kdp_max,ax,f] = kdp_objects_fig12(kdpc,KDPmasked,ax,f,time_start,month,d_beg,h_beg,min_beg,sec_beg,d_end,h_end,min_end,sec_end,rlons,rlats,max_lons_c,max_lats_c,kdplev,proj)
...
#                     print('Separation Angle Failure')
                ...
                if len(max_lons_c) > 0:
                    ax.scatter(max_lons_c,max_lats_c, marker = "o", color = 'r', s = 500, alpha = 1.0)
                ...
                plt.legend(handles=[zdr_outline, elevation], loc = 3, fontsize = 25)
                alt_levs = [1000, 2000]
                plt.savefig('Machine_Learning/ARCALG_testing'+station+str(time_start.year)+str(time_start.month)+str(day)+str(hour)+str(minute)+'.png')
                print('Figure Saved')
                plt.close()
